...
pid_t *get_process_list(uint32_t *count) {
    ...
    // kinfo_proc 结构体在 <sys/sysctl.h> 中定义
    // 通过 sysctl 直接获取
...
kern_return_t get_memory_regions(task_t task, MemoryRegion **regions, uint32_t *count) {
    ...
    *count = 0;
    ...
    while (true) {
        ...
        // iOS SDK 没有 is_mapped_file_share 字段
        region->is_mapped = 0;
        region->region_type = '?';

        if ((info.protection & VM_PROT_EXECUTE) && !(info.protection & VM_PROT_WRITE)) {
...
void print_memory_region(const MemoryRegion *region) {
    if (!region) return;
    printf("[%c] 0x%016llx - 0x%016llx [size: %lld] [prot: %c%c%c]\n",
           region->region_type,
...
static bool find_string_in_buffer(const uint8_t *buf, size_t buf_len,
    ...
    for (size_t i = 0; i <= buf_len - pattern_len; i++) {
        ...
        for (size_t j = 0; j < pattern_len; j++) {
            ...
            char pc = pattern[j];
            if (!case_sensitive) { bc = to_lower_char(bc); pc = to_lower_char(pc); }
            if (bc != pc) { found = false; break; }
        }
        if (found) { *out_addr = base_addr + i; *out_offset = i; return true; }
    }
    return false;
}
...
static bool add_result(ScanResultSet *set, mach_vm_address_t addr,
    ...
    if (item->context_len > sizeof(item->context)) item->context_len = sizeof(item->context);
...
ScanResultSet scan_process_memory(task_t task,
    ...
    if (!target_string || strlen(target_string) == 0) return result;
    ...
    // iOS 兼容的内存读取
    // 使用 vm_read_overwrite 替代 mach_vm_read_overwrite
    vm_map_t target_map = task;
    mach_vm_address_t address = 0;
    mach_vm_size_t size = 0;
    ...
    if (!buffer) { free(unicode_pattern); return result; }

    while (true) {
        vm_address_t vm_addr = (vm_address_t)address;
        vm_size_t vm_size = 0;
        natural_t depth = 0;
        struct vm_region_submap_info_64 info;
        mach_msg_type_number_t info_count = VM_REGION_SUBMAP_INFO_COUNT_64;

        kern_return_t kr = vm_region_recurse_64(target_map, &vm_addr, &vm_size, &depth,
                                                 (vm_region_recurse_info_t)&info, &info_count);
        if (kr != KERN_SUCCESS) { if (kr == KERN_NO_SPACE) break; break; }
        if (info.is_submap) { depth++; continue; }

        address = (mach_vm_address_t)vm_addr;
        size = (mach_vm_size_t)vm_size;

        // 区域过滤器
        bool pass_region = true;
        if (region_filter) {
            pass_region = false;
            switch (region_filter[0]) {
                case 'D': pass_region = (info.protection & VM_PROT_READ) && (info.protection & VM_PROT_WRITE); break;
                case 'T': pass_region = (info.protection & VM_PROT_EXECUTE) && !(info.protection & VM_PROT_WRITE); break;
                case 'R': pass_region = (info.protection & VM_PROT_READ) != 0; break;
                default:  pass_region = (info.protection & VM_PROT_READ) != 0; break;
            }
        } else {
            pass_region = (info.protection & VM_PROT_READ) != 0;
        }
        if (!pass_region) { address += size; depth = 0; info_count = VM_REGION_SUBMAP_INFO_COUNT_64; continue; }
        ...
        while (remaining > 0) {
            ...
            // 使用 vm_read_overwrite 读取目标进程内存（iOS 兼容）
            kr = vm_read_overwrite(target_map, (vm_address_t)current_addr,
                                    (vm_size_t)read_size, (vm_address_t)buffer, (vm_size_t *)&bytes_read);
            if (kr != KERN_SUCCESS) { current_addr += read_size; remaining -= read_size; continue; }

            size_t buf_len = (size_t)bytes_read;
            mach_vm_address_t found_addr = 0;
            size_t found_offset = 0;

            if (find_string_in_buffer(buffer, buf_len, target_string, target_len,
                                      case_sensitive, current_addr, &found_addr, &found_offset)) {
                add_result(&result, found_addr, buffer, found_offset, buf_len);
                if (max_results > 0 && result.count >= max_results) { free(buffer); free(unicode_pattern); return result; }
            }

            if (match_unicode && unicode_pattern) {
                if (find_string_in_buffer(buffer, buf_len, (const char *)unicode_pattern,
                                          unicode_pattern_len, case_sensitive,
                                          current_addr, &found_addr, &found_offset)) {
                    add_result(&result, found_addr, buffer, found_offset, buf_len);
                    if (max_results > 0 && result.count >= max_results) { free(buffer); free(unicode_pattern); return result; }
                }
            }
...
void free_scan_results(ScanResultSet *results) {
    if (!results) return;
    if (results->items) { free(results->items); results->items = NULL; }
    if (results->searched_string) { free(results->searched_string); results->searched_string = NULL; }
    results->count = 0; results->capacity = 0;
}
...
size_t read_process_memory(task_t task, mach_vm_address_t address, void *buffer, size_t size) {
    if (!buffer || size == 0) return 0;
    mach_vm_size_t bytes_read = 0;
    kern_return_t kr = vm_read_overwrite(task, (vm_address_t)address, (vm_size_t)size,
                                          (vm_address_t)buffer, (vm_size_t *)&bytes_read);
    return (kr == KERN_SUCCESS) ? (size_t)bytes_read : 0;
}

void print_scan_results(const ScanResultSet *results) {
    if (!results || !results->items) { printf("No results found.\n"); return; }
...
char *scan_results_to_json(const ScanResultSet *results) {
    ...
    for (uint32_t i = 0; i < results->count; i++) json_size += 128 + results->items[i].context_len * 4;
    ...
    int written = snprintf(ptr, remaining, "{\"searched_string\":\"%s\",\"count\":%u,\"results\":[",
                           results->searched_string ? results->searched_string : "", results->count);
    if (written > 0) { ptr += written; remaining -= written; }

    for (uint32_t i = 0; i < results->count; i++) {
        const ScanResultItem *item = &results->items[i];
        if (i > 0) { written = snprintf(ptr, remaining, ","); if (written > 0) { ptr += written; remaining -= written; } }
        written = snprintf(ptr, remaining, "{\"address\":\"0x%016llx\",\"context_hex\":\"", item->address);
        if (written > 0) { ptr += written; remaining -= written; }
        ...
        for (size_t j = 0; j < item->context_len && j < 64 && remaining > 0; j++) {
            char c = (char)item->context[j];
            if (c >= 32 && c <= 126 && c != '"' && c != '\\') { *ptr++ = c; remaining--; }
            else { written = snprintf(ptr, remaining, "\\x%02x", (unsigned char)c); if (written > 0) { ptr += written; remaining -= written; } }
        }
